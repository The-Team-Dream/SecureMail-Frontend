...
import type { Email, EmailDetails, Attachment } from "../../types/Email";
...
export const useReplyEmail = (mailboxId: string) => {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, formData }) => {
      ...
      // Get original email info from cache
      const originalEmail = queryClient.getQueryData<EmailDetails>([
        "email",
        id,
      ]);
      const toAddr = originalEmail ? [originalEmail.fromAddr] : [];
      const subject = originalEmail
        ? `Re: ${originalEmail.subject}`
        : "Reply Email";

      // Extract content & attachments from FormData
