...
import { Mail, Settings, Lock, Rocket, FileText, ArrowRight, ArrowLeft, ChevronRight } from "lucide-react";
...
export function AddAccountWizard({ onCancel, onSuccess }: AddAccountWizardProps) {
  const [step, setStep] = useState(1);
  const [provider, setProvider] = useState("Gmail");

  const { register, trigger, watch, setValue, reset, formState: { errors } } = useForm<WizardFormData>({
    ...
    <div className="flex flex-col w-full min-h-[calc(100vh-80px)] bg-card relative">
      ...
      <div className="flex flex-col flex-1 w-full mx-auto px-10 max-w-6xl pt-6">
        ...
        <div className={`flex flex-col mb-16 w-full mx-auto flex-1 mt-14 ${step === 5 ? 'max-w-[900px]' : 'max-w-[560px]'}`}>
          ...
          {step === 4 && <StepAdvanced formData={formData} handleChange={handleChange} />}
          {step === 5 && <StepSummary formData={formData} handleChange={handleChange} />}
        </div>
